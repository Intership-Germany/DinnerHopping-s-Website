  ...
  function renderEventCards(opts) {
    ...
    events.forEach((e) => {
      ...
      const fee = formatFeeCents
        ? formatFeeCents(typeof e.fee_cents === 'number' ? e.fee_cents : 0)
        : '';
        ...
        if (!fullDesc) {
          ...
          if (needsToggle) {
            ...
            requestAnimationFrame(() => {
              wrapper.style.maxHeight = measureHeight() + 'px';
            });
            let expanded = false;
      ...
      const capacity =
        e.capacity && Number.isInteger(e.capacity) && e.capacity > 0 ? e.capacity : 6;
      ...
      try {
        if (
          zipBadgeEl &&
          userZip &&
          Array.isArray(e.valid_zip_codes) &&
          e.valid_zip_codes.length > 0 &&
          !e.valid_zip_codes.includes(userZip)
        ) {
      ...
      if (refundEligible && metaWrap) {
        ...
        refundSpan.innerHTML =
          '<svg viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414L9 13.414l4.707-4.707z"/></svg><span>Refundable</span>';
      ...
      if (e.capacity && Number.isInteger(e.capacity) && e.capacity > 0) {
        const pct = Math.min(
          100,
          Math.max(0, ((Number(e.attendee_count) || 0) / e.capacity) * 100)
        );
      ...
      if (ctaEl) {
        ctaEl.href = '#';
        if (placeLeft <= 0 || (status && status !== 'open')) {
          ctaEl.classList.add('opacity-60', 'cursor-not-allowed');
          ctaEl.setAttribute('aria-disabled', 'true');
          ctaEl.tabIndex = -1;
          if (status === 'coming_soon') ctaEl.textContent = 'Coming soon';
          else if (status === 'closed' || status === 'ended') ctaEl.textContent = 'Closed';
        }
        ctaEl.addEventListener('click', (ev) => {
          ev.preventDefault();
          if (!eventId || ctaEl.getAttribute('aria-disabled') === 'true') return;
          if (typeof onRegister === 'function')
            onRegister({ event: e, eventId, spotsEl, ctaEl, placeLeft });
        });
      }
