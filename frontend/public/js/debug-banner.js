...
(function(){
  try {
    ...
    var env = (typeof window !== 'undefined' && window.APP_ENV) || 'unknown';
  ...
  var lastProfileStatus = null; // HTTP status or error code string
    ...
    function detectAuthMode(){
      try {
        ...
        var lsToken = null;
        try { lsToken = window.localStorage ? window.localStorage.getItem('dh_access_token') : null; } catch {}
        if (hasAccessCookie || hasRefreshCookie) { authMode = 'cookie'; bearerSource = null; return; }
        if (lsToken) { authMode = 'bearer-ls'; bearerSource = 'localStorage'; return; }
        if (dhCookieMatch) { authMode = 'bearer-dh'; bearerSource = 'dh_cookie'; return; }
        authMode = 'none'; bearerSource = null;
      } catch { authMode = 'unknown'; }
    }
    detectAuthMode();
    detectCsrf();
    detectSecureCookies();

    window.addEventListener('online', function(){ online = true; render(); });
    window.addEventListener('offline', function(){ online = false; render(); });

    function dot(color, title){
      return '<span title="'+(title||'')+'" style="display:inline-block;width:8px;height:8px;border-radius:50%;background:'+color+';margin-right:4px;box-shadow:0 0 2px rgba(0,0,0,.6);"></span>';
    }

    function statusDot(type){
      switch(type){
        case 'ok': return dot('#22c55e','OK'); // green
        case 'fail': return dot('#ef4444','Fail'); // red
        case 'pending': return dot('#facc15','Pending'); // yellow
        default: return dot('#9ca3af','Unknown'); // gray
      }
    }
    ...
    function render(){
      ...
      var roleDotColor = (roleStr.indexOf('admin') !== -1) ? '#6366f1' : (roleStr==='anonymous'? '#9ca3af':'#0ea5e9');
      ...
      var authColorMap = { 'cookie':'#22c55e', 'bearer-ls':'#0ea5e9', 'bearer-dh':'#f97316', 'none':'#ef4444', 'unknown':'#9ca3af' };
      var authColor = authColorMap[authMode] || '#9ca3af';
      var authLabel = (function(m){
        switch(m){
          case 'cookie': return 'cookie';
          case 'bearer-ls': return 'bearer(ls)';
          case 'bearer-dh': return 'bearer(dh_token)';
      ...
      if(collapsed){
        el.innerHTML = '<div style="display:flex;align-items:center;gap:4px;pointer-events:auto;">'
          ...
          +'<span style="opacity:.8;">'+escapeHtml(env)+'</span>'
          +'</div>';
        wireEvents();
        return;
      }
      ...
      el.innerHTML = ''+
        ...
        +'<div style="display:flex;align-items:center;gap:6px;">'
          ...
          +dot('#22c55e','Frontend origin')+'<span>FE: '+escapeHtml(origin)+'</span>'
          +'<span style="opacity:.7;">('+escapeHtml(env)+')</span>'
        +'</div>'
        +'<div style="white-space:nowrap;">'+statusDot(backendStatus)+'<span title="'+backendTitle+'">BE: '+backendDisplay+latencyPart+'</span></div>'
        +'<div style="white-space:nowrap;">'+dot(roleDotColor,'Roles')+'User: '+escapeHtml(roleStr)+(emailDisplay? ' ('+escapeHtml(emailDisplay)+')':'')+'</div>'
        +'<div style="white-space:nowrap;">'+dot(csrfColor,'CSRF token '+(csrfPresent?'present':'absent'))+'CSRF: '+(csrfPresent? 'present':'absent')+'</div>'
        +'<div style="white-space:nowrap;">'+dot(onlineColor,'Navigator online state')+'Net: '+(online? 'online':'offline')+'</div>'
        +'<div style="white-space:nowrap;">'+dot(secureColor,'Secure cookie heuristic (__Host-)')+'Cookies: '+(secureCookies? 'secure':'insecure/dev')+'</div>'
        +'<div style="white-space:nowrap;">'+dot(authColor,'Auth mode')+'Auth: '+authLabel+'</div>'
    ...
    function pingBackend(){
      ...
      fetch(base.replace(/\/$/,'') + '/openapi.json', { method:'GET', mode:'cors', cache:'no-store', signal: controller.signal })
        .then(function(r){
        ...
        .catch(function(){ backendStatus = 'fail'; lastLatencyMs = null; })
        .finally(function(){ clearTimeout(to); render(); });
    }
    ...
    function fetchProfile(){
      ...
      var lsBearer = null;
      try { lsBearer = (window.localStorage && window.localStorage.getItem('dh_access_token')) || null; } catch{}
      var dhBearer = null; var m = document.cookie.match(/(?:^|; )dh_token=([^;]+)/); if(m) dhBearer = decodeURIComponent(m[1]);
      ...
      function doBearer(token){
        ...
        triedBearer = true;
        return fetch(url, { method:'GET', credentials:'omit', headers:{'Accept':'application/json','Authorization':'Bearer '+token} })
          .then(function(r){ lastProfileStatus = r.status; return r; })
          .catch(function(e){ lastProfileStatus = 'bearer_err'; return null; });
      }
      function doCookie(){
        triedCookie = true;
        return fetch(url, { method:'GET', credentials:'include', headers:{'Accept':'application/json'} })
          .then(function(r){ lastProfileStatus = r.status; return r; })
          .catch(function(e){ lastProfileStatus = 'cookie_err'; return null; });
      }
      ...
      chain
        ...
        .finally(function(){ detectAuthMode(); render(); });
    }
    ...
    function snapshot(){
      ...
      try {
        ...
        setTimeout(function(){ el.style.boxShadow = '0 4px 12px rgba(0,0,0,0.25)'; }, 600);
      } catch(e){ console.warn('snapshot copy failed', e); }
    }
    ...
    function wireEvents(){
      ...
      if(btnToggle){
        btnToggle.onclick = function(ev){ ev.stopPropagation(); collapsed = !collapsed; render(); };
      }
      var btnSnap = el.querySelector('[data-act="snapshot"]');
      if(btnSnap){ btnSnap.onclick = function(ev){ ev.stopPropagation(); snapshot(); }; }
      var btnRef = el.querySelector('[data-act="refresh-prof"]');
      if(btnRef){ btnRef.onclick = function(ev){ ev.stopPropagation(); manualRefreshProfile(); }; }
    }
      ...
      if (window.BACKEND_BASE_URL === undefined) {
        ...
        var iv = setInterval(function(){
          ...
          if (window.BACKEND_BASE_URL !== undefined) {
            ...
            console.warn('[debug-banner] BACKEND_BASE_URL still undefined after', attempts, 'attempts');
