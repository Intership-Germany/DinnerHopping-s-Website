document.addEventListener('DOMContentLoaded', () => {
  ...
  function setMsg(text, type='info'){
    ...
    msg.className = 'mb-4 text-sm text-center ' + (type==='error' ? 'text-red-600' : 'text-green-700');
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const p = pwd.value;
    const c = conf.value;
    if (!p || !c) { setMsg('Please fill both password fields.', 'error'); return; }
    if (p !== c) { setMsg('Passwords do not match.', 'error'); return; }
    if (p.length < 8) { setMsg('Password must be at least 8 characters.', 'error'); return; }
    try {
      ...
      if (!res.ok) {
        const detail = data && data.detail ? (Array.isArray(data.detail) ? data.detail.map(d=>d.msg).join(' ') : data.detail) : 'Unable to reset password.';
        setMsg(detail, 'error');
        if (btn) { btn.disabled = false; btn.textContent = 'Set new password'; }
        return;
      }
      setMsg('Your password has been reset. You can now log in with your new password. Redirecting…');
