...
def ensure_admin():
    ...
    try:
        ...
        with httpx.Client(timeout=20.0) as c:
            r = c.post(f"{BASE_URL}/login", json={"username": ADMIN_EMAIL, "password": ADMIN_PASSWORD})
            if r.status_code == 200:
                return r.json().get("access_token")
    except Exception:
        pass

    # create admin user via /register (idempotent in the sense tests create directly in DB but we use API)
    try:
        _rate_limit()
        with httpx.Client(timeout=20.0) as c:
            payload = {
            ...
            r = c.post(f"{BASE_URL}/register", json=payload)
            # Accept 201 or 400 conflicts
    except Exception:
        pass

    # Try to promote to admin using /admin/promote requires existing admin; fallback: if backend supports direct DB-only admin creation, tests do it but we cannot here.
    # We'll attempt login repeatedly and if fail, inform user to create admin manually.
    for _ in range(6):
        try:
            _rate_limit()
            with httpx.Client(timeout=20.0) as c:
...
def create_events(admin_token: str, n: int = 10) -> List[dict]:
    ...
    with auth_client(admin_token) as c:
        for i in range(n):
            ...
            capacity = random.choice([20, 30, 40, None])
            payload = {
                "title": title,
            ...
            if r.status_code in (200, 201):
                ...
                print("WARN: create event failed", r.status_code, r.text)
    return events
...
def register_users_from_rows(rows: List[Dict]):
    ...
    print(f"Users created/attempted: {created}")
